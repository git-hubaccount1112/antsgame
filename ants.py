...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        ant_move = self.place
        count = 0
        while ant_move is not beehive:
            if ant_move.bees:
                if count >= self.min_range and count <= self.max_range:
                    return random_or_none(ant_move.bees)
            ant_move = ant_move.entrance
            count += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        list_of_bees = list(bees_in_place)
    #    for i in range(len(list_of_bees)):
    #        if self.armor < 3:
    #            list_of_bees[i].armor = .reduce_armor(amount)
    #        if armor <= 0:
    #            list_of_bees[i].armor = bees_in_place.armor - 3
        self.armor -= amount
        for i in range(len(list_of_bees)):
            if self.armor > 0:
                list_of_bees[i].reduce_armor(amount)
            elif self.armor <= 0:
                list_of_bees[i].reduce_armor(amount + self.damage)
        if self.armor <= 0:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        Ant.__init__(self, armor)
        if QueenAnt.fake_queen == False: # real queen
            self.fake_queen = False
            QueenAnt.fake_queen = True
        else: # fake ones
    ...
    def action(self, gamestate):
        ...
        # self.throw_at(self.place.bees) #damage bees
        # if counter > 1:
        #     self.reduce_armor(self)
        elif self.fake_queen == False:
            ThrowerAnt.action(self, gamestate)
            place = self.place.exit
            while place:
                if place.ant and place.ant not in self.buff_ants:
                    place.ant.damage *= 2
                    self.buff_ants.append(place.ant)
                if place.ant and place.ant.is_container and place.ant.contained_ant and place.ant.contained_ant not in self.buff_ants:
                    place.ant.contained_ant.damage *= 2
                    self.buff_ants.append(place.ant.contained_ant)
                place = place.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor -= amount
        if self.armor <= 0 and self.fake_queen == True:
            self.place.remove_insect(self)
            self.death_callback()
        else:
            bees_win()
